...
func (x Fixed) integer() int64 {
	if x.bitlen() > 56+63 { panic(ErrOverflow) }
...
func (x Fixed) floor() int64 {
	...
	if sign < 0 && x.lo&uint64(fracMask) != 0 { return v-1 }
	return v
}
...
func (x Fixed) bit(i int) bool {
	switch {
	case i > 127: return false
	case i >= 64: return (x.hi>>(i-64))&1 != 0
	case i >= 0: return (x.hi>>i)&1 != 0
	}
	return false
}
